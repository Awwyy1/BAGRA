...
const StarIcon = () => (
  <svg className="w-3.5 h-3.5 md:w-3 md:h-3 fill-white/60" viewBox="0 0 20 20">
          ...
          {masters.map((master, idx) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 md:p-6 bg-gradient-to-t from-black/90 to-transparent">
                <h3 className="text-2xl md:text-2xl font-bold tracking-tight font-heading mb-1">{master.name}</h3>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 md:gap-2">
                    <FiveStars />
                    <span className="text-xs md:text-[10px] font-mono opacity-50">{master.reviews}</span>
                  </div>
                </div>
              </div>
            </motion.div>
